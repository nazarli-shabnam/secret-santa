...
app.post("/api/bootstrap", (req, res) => {
  ...
  if (state.players.length > 0) {
    ...
    return res
      .status(400)
      .json({ error: "Players already exist. Use admin panel to add more." });
  }
...
app.get("/api/game/status", validateSession, (req, res) => {
  ...
  const playerStatus = gameState.playerStatus.get(name);
  const status = playerStatus || { hasSpun: false, spinCount: 0 };
  ...
  } else if (!playerStatus && !gameState.availablePool.includes(name)) {
    // Player added mid-game but not in pool (all assignments made)
    message =
      "All assignments have already been made. You cannot spin in this round.";
  } else if (status.hasSpun) {
    if (isDoubleSpinner && status.spinCount < 2) {
      canSpin = true;
  ...
  res.json({
    ...
    hasSpun: status.hasSpun,
    isDoubleSpinner:
      isDoubleSpinner && !status.hasSpun
        ? true
        : isDoubleSpinner && status.spinCount < 2,
  });
});
...
app.post("/api/spin", validateSession, (req, res) => {
  ...
  // Check if player is in the game (in playerStatus or pool)
  const playerStatus = gameState.playerStatus.get(name);
  if (!playerStatus && !gameState.availablePool.includes(name)) {
    return res
      .status(400)
      .json({ error: "You are not participating in this game" });
  }

  const status = playerStatus || { hasSpun: false, spinCount: 0 };
  const isDoubleSpinner = gameState.doubleSpinner === name;

  if (status.hasSpun && !(isDoubleSpinner && status.spinCount < 2)) {
  ...
  // Return formatted assignment (handle arrays for double-spinner)
  let responseAssignment = assignedTo;
  const finalAssignment = gameState.assignments.get(name);
  if (Array.isArray(finalAssignment)) {
    responseAssignment = finalAssignment.join(" and ");
  }

  res.json({ assignedTo: responseAssignment });
});
...
app.post("/api/admin/players", validateSession, requireAdmin, (req, res) => {
  ...
  const playerRole = role || "user";
  state.players.push({ name, password, role: playerRole });

  // If game is started, add player to status and pool
  // Note: We add all new players to the pool, allowing them to participate
  // Admin can control participation via the "admin participates" checkbox when starting
  if (state.gameState.started) {
    // Add to playerStatus
    state.gameState.playerStatus.set(name, { hasSpun: false, spinCount: 0 });

    // Add to pool so they can spin (if pool isn't empty)
    // This allows mid-game additions to participate
    state.gameState.availablePool.push(name);
  }
...
app.delete(
  ...
  (req, res) => {
    ...
    if (state.gameState.started) {
      return res
        .status(400)
        .json({ error: "Cannot delete players after game has started" });
    }
...
app.post("/api/admin/start", validateSession, requireAdmin, (req, res) => {
  ...
  if (playerCount % 2 === 1) {
    if (!doubleSpinnerName) {
      return res
        .status(400)
        .json({ error: "Odd number of players — select a double-spinner" });
    }
    ...
    if (!pool.includes(doubleSpinnerName)) {
      return res
        .status(400)
        .json({ error: "Double-spinner must be a participating player" });
    }
