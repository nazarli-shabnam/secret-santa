...
function setupEventListeners() {
    ...
    document.getElementById('adminParticipates').addEventListener('change', updateDoubleSpinnerSection);
}
    ...
    try {
        ...
        if (response.ok) {
            ...
            try {
                ...
                errorDiv.textContent = 'Admin created but login failed. Please try logging in manually.';
            ...
            if (loginResponse.ok && loginData.success) {
                ...
                errorDiv.textContent = loginData.error || 'Admin created but login failed. Please try logging in.';
            }
        } else {
            errorDiv.textContent = data.error || `Failed to create admin (${response.status})`;
        ...
        data.players.forEach(player => {
            ...
            item.innerHTML = `
                <div>
                    ...
                    ${player.role === 'admin' ? '<span class="player-role"> (Admin)</span>' : ''}
                </div>
...
async function loadGameControl() {
    try {
        ...
        const adminParticipates = document.getElementById('adminParticipates').checked;
        
        const effectiveCount = adminParticipates ? playerCount : playerCount - 1;
        const isOdd = effectiveCount % 2 === 1;
        
        const doubleSpinnerSection = document.getElementById('doubleSpinnerSection');
...
async function handleDeletePlayer(name) {
    try {
        const response = await fetch(`${API_BASE}/admin/players/${encodeURIComponent(name)}`, {
            method: 'DELETE',
            credentials: 'include'
        });
...
async function handleStartGame() {
    const adminParticipates = document.getElementById('adminParticipates').checked;
    const doubleSpinnerName = document.getElementById('doubleSpinnerSelect').value;
...
async function handleResetGame() {
    if (!confirm('Are you sure you want to reset the game? This will clear all assignments.')) {
        return;
    }
